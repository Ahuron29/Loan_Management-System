	...
	<form action="" id="loan-application">
		...
		<div class="row">
			<div class="col-md-6">
				...
				<?php
				$borrower = $conn->query("SELECT *,concat(name, ' ', username) as name FROM borrowers order by concat('name', ',username,') asc ");
				?>
				<select name="borrower_id" id="borrower_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $borrower->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($borrower_id) && $borrower_id == $row['id'] ? "selected" : '' ?>><?php echo $row['name'] . ' | ID:'.$row['voters_id'] ?></option>
						<?php endwhile; ?>
				</select>
			</div>
			<div class="col-md-6">
				...
				<select name="plan_id" id="plan_id" class="custom-select browser-default select2">
					<option value=""></option>
						<?php while($row = $plan->fetch_assoc()): ?>
							<option value="<?php echo $row['id'] ?>" <?php echo isset($plan_id) && $plan_id == $row['id'] ? "selected" : '' ?> data-months="<?php echo $row['months'] ?>" data-interest_percentage="<?php $month = $row['months']; $day = 4; $week1 = $month*$day; $weekly=$week1; echo $row['interest_percentage'] ?>" data-penalty_rate="<?php echo $row['penalty_rate'] ?>"><?php echo $row['months'] . ' month/s [ '.$row['interest_percentage'].'%, '.$weekly. 'Weeks'.']' ?></option>
						<?php endwhile; ?>
				</select>
				<small>months [ interest and Weeks ]</small>
			</div>
...
<script>
	...
	function calculate(){
		...
		if($('#loan_plan_id').val() == '' && $('[name="amount"]').val() == ''){
			alert_toast("Select plan and enter amount user.","warning");
			return false;
		}
