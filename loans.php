		...
		<div class="card">
			<div class="card-header">
				<large class="card-title">
					<button style="margin:-5px;" class="btn btn-primary btn-md btn-block col-md-3 float-left" type="button" id="push_alert"><i  class="fa fa-plus"></i> Push email alert to client</button>
				...
				<div class="text-center"><b>Loan List</b></div>
			</div>
			<div class="card-body">
				<table class="table table-bordered" id="loan-list">
					...
					<tbody>
						<?php
							$bal = $conn->query("SELECT l.*,concat(b.name)as name, b.email, b.voters_id, b.username, b.contact, b.address from loan_list l inner join payments p on p.id = l.loan_id  order by id asc");
							$i=1;
							$type = $conn->query("SELECT * FROM loan_types where id in (SELECT loan_type_id from loan_list) ");
							while($row=$type->fetch_assoc()){
								$type_arr[$row['id']] = $row['type_name'];
							}
							$plan = $conn->query("SELECT *,concat(months,' month/s [ ',interest_percentage,'%, ]') as plan FROM loan_plan where id in (SELECT plan_id from loan_list) ");
							while($row=$plan->fetch_assoc()){
								$plan_arr[$row['id']] = $row;
							}
							$qry = $conn->query("SELECT l.*,concat(b.name)as name, b.email, b.voters_id, b.username, b.contact, b.address from loan_list l inner join borrowers b on b.id = l.borrower_id  order by id asc");
							while($row = $qry->fetch_assoc()):
								...
								if($row['status'] == 2):
									$next = $conn->query("SELECT * FROM loan_schedules where loan_id = '".$row['id']."'  order by date(date_due) asc limit 1")->fetch_assoc()['date_due'];
								endif;
								$sum_paid = 0;
						 ...
						 <tr>
						 	...
						 	<td>
						 		...
						 		<p><small>Amount : Ghs<b><?php echo " ".$row['amount'] ?></small></b></p>
						 		<p><small>Total Payable Amount: <b><?php echo number_format($monthly * $plan_arr[$row['plan_id']]['months'],2) ?></small></b></p>
						 		<p><small>Weekly Amount: Ghs<b><?php echo " ".number_format($monthly/4,2) ?></small></b></p>
						 		<?php if($row['status'] == 2 || $row['status'] == 3): ?>
						 		<p><small>Date Released: <b><?php echo date("M d, Y",strtotime($row['date_released'])) ?></small></b></p>
						 		<?php endif; ?>
						 	</td>
							  <td>
						 		<p><small>Total Loan: <b><?php echo $row['amount'] ?></small></b></p>
								 <p><small>Amount Paid: <?php //echo $row['username'] ?></small></p>
								 <p><small>Loan Balance: <?php //echo $row['email'] ?></small></p>
								 <p><small>Total Savings: <?php //echo $row['voters_id'] ?></small></p>
						 		<p><small>Number installment: <b><?php //echo $row['contact'] ?></small></b></p>
						 		<p><small>Number of Savings: <b><?php //echo $row['address'] ?></small></b></p>
						 	</td>
...
<script>
	$('#loan-list').dataTable()
	$('#push_alert').click(function(){
		uni_modal("Send Email to Client","push_alert.php",'mid-large')
	})
