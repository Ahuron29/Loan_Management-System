...
Class Action {
	...
	function save_group(){
		...
		if(empty($id)){
			$save = $this->db->query("INSERT INTO groups set ".$data);
		}else{
			$save = $this->db->query("UPDATE groups set ".$data." where id=".$id);
		}
		if($save)
			return 1;
	}
	function delete_group(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM groups where id = ".$id);
		if($delete)
			return 1;
	}
	function save_type(){
		...
		extract($_POST);
			...
			if(isset($status)){
				...
				if($status == 2){
					...
					if($chk > 0){
						$thi->db->query("DELETE FROM loan_schedules where  = $id ");
					}
